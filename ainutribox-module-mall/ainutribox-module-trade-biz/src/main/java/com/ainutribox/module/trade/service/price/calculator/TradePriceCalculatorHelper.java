...
import static com.ainutribox.framework.common.util.collection.CollectionUtils.getSumValue;
...
public class TradePriceCalculatorHelper {
    ...
    public static TradePriceCalculateRespBO buildCalculateResp(TradePriceCalculateReqBO param,
        ...
        param.getItems().forEach(item -> {
            ...
            // spu 信息
            orderItem.setSpuName(item.getSpuName()).setCategoryId(spu.getCategoryId())
                    .setDeliveryTemplateId(spu.getDeliveryTemplateId())
                    .setGivePoint(spu.getGiveIntegral()).setUsePoint(0)
                    .setPicUrl(spu.getPicUrl());
            //附加信息
            orderItem.setItemType(item.getItemType()).setItemNumber(item.getItemNumber())
                    .setDietitianSpuId(item.getDietitianSpuId()).setDietitianId(item.getDietitianId()).setSpuUnit(item.getSpuUnit());
