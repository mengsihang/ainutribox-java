package com.ainutribox.module.trade.convert.order;

import cn.hutool.core.util.BooleanUtil;
import cn.hutool.core.util.StrUtil;
import com.ainutribox.framework.common.pojo.PageResult;
import com.ainutribox.framework.common.util.collection.CollectionUtils;
import com.ainutribox.framework.common.util.string.StrUtils;
import com.ainutribox.framework.dict.core.DictFrameworkUtils;
import com.ainutribox.framework.ip.core.utils.AreaUtils;
...
import com.ainutribox.module.product.api.property.dto.ProductPropertyValueDetailRespDTO;
import com.ainutribox.module.product.api.sku.dto.ProductSkuRespDTO;
import com.ainutribox.module.product.api.sku.dto.ProductSkuUpdateStockReqDTO;
import com.ainutribox.module.product.api.spu.dto.ProductSpuRespDTO;
import com.ainutribox.module.promotion.api.combination.dto.CombinationRecordCreateReqDTO;
import com.ainutribox.module.trade.api.order.dto.TradeOrderRespDTO;
import com.ainutribox.module.trade.controller.admin.base.member.user.MemberUserRespVO;
import com.ainutribox.module.trade.controller.admin.base.product.property.ProductPropertyValueDetailRespVO;
import com.ainutribox.module.trade.controller.admin.order.vo.*;
import com.ainutribox.module.trade.controller.app.base.property.AppProductPropertyValueDetailRespVO;
import com.ainutribox.module.trade.controller.app.order.vo.*;
import com.ainutribox.module.trade.controller.app.order.vo.item.AppTradeOrderItemCommentCreateReqVO;
import com.ainutribox.module.trade.controller.app.order.vo.item.AppTradeOrderItemRespVO;
...
import com.ainutribox.module.trade.dal.dataobject.order.TradeOrderLogDO;
import com.ainutribox.module.trade.enums.order.TradeOrderItemAfterSaleStatusEnum;
import com.ainutribox.module.trade.framework.delivery.core.client.dto.ExpressTrackRespDTO;
import com.ainutribox.module.trade.framework.order.config.TradeOrderProperties;
import com.ainutribox.module.trade.service.brokerage.bo.BrokerageAddReqBO;
...
public interface TradeOrderConvert {
        ...
        for (AppTradeOrderSettlementReqVO.Item item : settlementReqVO.getItems()) {
            ...
            if (item.getSkuId() != null) {
                reqBO.getItems().add(new TradePriceCalculateReqBO.Item().setSkuId(item.getSkuId()).setCount(item.getCount())
                        .setSelected(true).setSpuName(item.getSpuName()).setItemType(item.getItemType()).setDietitianId(item.getDietitianId())
                        .setItemNumber(item.getItemNumber()).setDietitianSpuId(item.getDietitianSpuId()).setSpuUnit(item.getSpuUnit())); // true 的原因，下单一定选中
                continue;
            }
            // 情况二：cartId
    ...
    default BrokerageAddReqBO convert(MemberUserRespDTO user, TradeOrderItemDO item,
        ...
        BrokerageAddReqBO bo = new BrokerageAddReqBO().setBizId(String.valueOf(item.getId())).setSourceUserId(item.getUserId())
                //.setBasePrice(item.getPayPrice() * item.getCount()) //这里有疑问 payPrice 已经是总价了 为什么还要乘以数量
